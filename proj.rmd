...
# Data Access, loading librarirs
>>>>>>> fc7c34a8fab49cf8fbea0673dcac17d2cceed0aa
```{r}
library(tidyverse)
# Summarize the data
library(knitr)  
...
<<<<<<< HEAD
=======
# Cleaning missing data and data transformation
>>>>>>> fc7c34a8fab49cf8fbea0673dcac17d2cceed0aa
df <- data %>%
  mutate(
...
# Convert the 'Drinks_Consumed' and 'Classes_Missed' columns in the dataframe 'df' to factor type.
# This is typically done to prepare categorical data for statistical tests and visualizations.
df$Drinks_Consumed <- as.factor(df$Drinks_Consumed)
df$Classes_Missed <- as.factor(df$Classes_Missed)


# Perform a Chi-square test of independence to evaluate if there is a significant association
# between the number of drinks consumed and classes missed. This is a common test for categorical data.
chi_square_test <- chisq.test(df$Drinks_Consumed, df$Classes_Missed)
chi_square_test

print(df$Classes_Missed)
# Create a hexbin plot using ggplot2 package, plotting 'Drinks_Consumed' on the x-axis and 'Classes_Missed' on the y-axis.
# Hexbin plots are useful for visualizing the density of data points when there are many overlapping points in a scatter plot.
>>>>>>> fc7c34a8fab49cf8fbea0673dcac17d2cceed0aa
